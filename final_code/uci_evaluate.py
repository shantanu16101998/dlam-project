...
def evaluate(args):
    ...
    for col in series_names:
        ...
        for start in range(0, len(test_normalized), FORECAST_LENGTH,):
            ...
            x = np.asarray(history[-BACKCAST_LENGTH:], dtype=np.float32,)
    ...
    result_mae = mae(all_true, all_pred,)
    result_rmse = rmse(all_true, all_pred,)
    result_mape = mape(all_true, all_pred,)
    ...
    results = pd.DataFrame({"metric": ["MAE", "RMSE", "MAPE",], "value": [result_mae, result_rmse, result_mape,],})
...
if __name__ == "__main__":
    ...
    parser.add_argument("--checkpoint", default="./uci_nbeats.pt",)
    parser.add_argument("--output", default="./uci_results.csv",)
